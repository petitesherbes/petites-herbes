...
export default function RappelsPage() {
  ...
  async function charger() {
    const [{ data: lignesData }, { data: especesData }, { data: semisData }] = await Promise.all([
      supabase
        .from('semis_lignes')
  ...
  const grainesUrgentes = stockGraines.filter(e => e.stock_actuel_g > 0 && e.stock_actuel_g < 200)
  const grainesSurveiller = stockGraines.filter(e => e.stock_actuel_g >= 200 && e.stock_actuel_g < 500)
  ...
  return (
    <div className="p-4 space-y-4">
        ...
        <div className="rounded-xl border border-orange-200 overflow-hidden shadow-sm">
          ...
          <div className="bg-orange-50 divide-y divide-orange-100">
            {grainesSurveiller.map(e => (
              <div key={e.id} className="px-3 py-2.5 flex justify-between items-center text-sm">
                <span className="font-medium">{e.nom}</span>
                <span className="text-orange-700">{e.stock_actuel_g}g</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {toutVide && grainesUrgentes.length === 0 && (
