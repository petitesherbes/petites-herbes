...
// Colonnes éditables (champs stockés en base)
type EditCol = { key: keyof Espece; label: string; unit?: string; defaultOn: boolean }
// Colonnes calculées (affichage seul)
type CalcCol = { key: string; label: string; unit?: string; defaultOn: boolean; calc: (e: Espece) => number | null }
type AnyCol = ({ kind: 'edit' } & EditCol) | ({ kind: 'calc' } & CalcCol)

const EDIT_COLS: EditCol[] = [
  ...
  { key: 'nb_tapis_serie', label: 'Tapis/série',       unit: 'tapis',   defaultOn: true  },
...
const CALC_COLS: CalcCol[] = [
  {
    key: 'g_serie_tapis',
    label: 'g/série tapis',
    unit: 'g',
    defaultOn: true,
    calc: (e) => (e.g_tapis !== null && e.nb_tapis_serie !== null)
      ? Math.round(e.g_tapis * e.nb_tapis_serie)
      : null,
  },
]

// Toutes les colonnes dans l'ordre d'affichage
const TOUTES_COLS: AnyCol[] = [
  ...EDIT_COLS.map(c => ({ kind: 'edit' as const, ...c })),
  ...CALC_COLS.map(c => ({ kind: 'calc' as const, ...c })),
]

const STORAGE_KEY = 'fiche_semis_cols'

function initCols(): string[] {
  if (typeof window === 'undefined') return TOUTES_COLS.filter(c => c.defaultOn).map(c => c.key as string)
  try {
    const saved = localStorage.getItem(STORAGE_KEY)
    if (saved) return JSON.parse(saved) as string[]
  } catch { /* */ }
  return TOUTES_COLS.filter(c => c.defaultOn).map(c => c.key as string)
}
...
export default function FicheSemisPage() {
  ...
  function handleKey(ev: React.KeyboardEvent) {
    if (ev.key === 'Enter') sauvegarder()
    if (ev.key === 'Escape') setEditCell(null)
  }

  const colsVisibles = TOUTES_COLS.filter(c => colsActives.includes(c.key as string))
      ...
      <div className="print:hidden sticky top-0 z-20 bg-white border-b border-gray-200 px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="text-xs text-gray-400">Cliquer sur une cellule pour la modifier • g/série tapis = calculé auto</div>
          </div>
        ...
        {configOpen && (
          <div className="mt-3 pt-3 border-t border-gray-100">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {TOUTES_COLS.map(c => {
                const on = colsActives.includes(c.key as string)
                const isCalc = c.kind === 'calc'
                return (
                  <button key={c.key as string} onClick={() => toggleCol(c.key as string)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors
                      ${on
                        ? isCalc ? 'bg-blue-600 text-white border-blue-600' : 'bg-green-700 text-white border-green-700'
                        : 'bg-white text-gray-400 border-gray-200 line-through'}`}>
                    {on ? '✓' : '○'} {c.label}
                    {isCalc && <span className="text-[9px] opacity-70">auto</span>}
                  </button>
                )
              })}
            </div>
            <div className="flex gap-2 mt-2">
              <button onClick={() => {
                const all = TOUTES_COLS.map(c => c.key as string)
                setColsActives(all); localStorage.setItem(STORAGE_KEY, JSON.stringify(all))
              }} className="text-xs text-green-700 underline">Tout cocher</button>
              <span className="text-gray-300">|</span>
              <button onClick={() => {
                setColsActives([]); localStorage.setItem(STORAGE_KEY, JSON.stringify([]))
              }} className="text-xs text-red-500 underline">Tout décocher</button>
            </div>
            <div className="mt-2 text-[10px] text-blue-500">
              🔵 Colonnes <strong>auto</strong> = calculées (non éditables directement)
            </div>
          </div>
        )}
      </div>
              ...
              <tr className="bg-gray-800 text-white">
                ...
                {colsVisibles.map(c => (
                  <th key={c.key as string}
                    className={`px-3 py-2.5 text-center whitespace-nowrap min-w-[75px] ${c.kind === 'calc' ? 'text-blue-200' : ''}`}>
                    {c.label}
                    {c.unit && <div className="text-[8px] font-normal opacity-60 leading-tight">{c.unit}</div>}
                    {c.kind === 'calc' && <div className="text-[8px] font-normal text-blue-300 leading-tight">auto</div>}
                  </th>
                ))}
              </tr>
            </thead>
                ...
                <tr key={e.id}
                  ...
                  {colsVisibles.map(c => {
                    if (c.kind === 'calc') {
                      const val = c.calc(e)
                      return (
                        <td key={c.key} className="px-2 py-1.5 text-center text-blue-700 font-semibold bg-blue-50/40 print:bg-blue-50">
                          {val !== null ? val : <span className="text-gray-200 text-[9px]">—</span>}
                        </td>
                      )
                    }
                    ...
                    return (
                      <td key={c.key as string} className="px-1 py-1 text-center text-gray-700">
                        {isEditing ? (
                          <input
                            autoFocus
                            value={editVal}
                          ...
                          <button
                            onClick={() => ouvrirEdit(e.id, c.key as keyof Espece, val)}
                            ...
                            {val !== null ? val : <span className="text-[9px]">—</span>}
                          </button>
                        )}
                      </td>
                    )
