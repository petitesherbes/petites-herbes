...
export default function CommanderPage() {
    ...
    const observer = new IntersectionObserver(
      entries => {
        for (const e of entries) {
          if (e.isIntersecting) {
            setCatActive(e.target.getAttribute('data-cat') as ProduitCategorie)
          }
        }
      },
  ...
  // Scroll la nav vers la tab active
  useEffect(() => {
  ...
  function scrollTocat(cat: ProduitCategorie) {
    ...
    if (!el) return
    // offset = hauteur du header (~120px) + nav (~52px) = 172px
  ...
  function setQte(produitId: string, delta: number) {
    ...
    setPanier(prev => {
      ...
      if (nouveau === 0) { const { [produitId]: _, ...rest } = prev; return rest }
  ...
  async function commander() {
    ...
    setSending(false)
    if (res.ok) { setBlNumero(data.numero); setEcran('confirmation') }
    else alert('Erreur lors de la commande. Veuillez réessayer.')
  }
      ...
      <div className="p-5 space-y-3 max-w-lg mx-auto">
        ...
        <div className="bg-green-50 rounded-2xl p-4">
          <p className="text-sm text-green-800 leading-relaxed">
            ...
            Vous recevrez un email de confirmation avec votre bon de livraison.
          </p>
        </div>
        <button
  ...
  return (
    <div className="min-h-screen bg-[#F7F5F0]" style={{ paddingBottom: nbArticles > 0 ? '100px' : '32px' }}>
      ...
      <div className="sticky top-0 z-20">
        {/* Bande verte */}
        <div className="bg-green-900 px-5 py-3 flex items-center gap-3">
        ...
        {/* Nav catégories scrollable */}
        {categories.length > 1 && (
          <div ref={catNavRef}
            className="bg-white border-b border-gray-100 flex gap-1 overflow-x-auto px-3 py-2 no-scrollbar">
            {categories.map(cat => (
              <button
                key={cat}
                data-cat={cat}
      ...
      {/* ── Contenu ── */}
      <div className="max-w-lg mx-auto px-4 pt-5 space-y-7">
                  ...
                  return (
                    <div key={p.id}
                      ...
                      <div className="relative w-full aspect-[4/3] bg-gradient-to-br from-green-50 to-green-100 overflow-hidden">
                        ...
                        {p.bio && (
                          <div className="absolute top-1.5 left-1.5 bg-green-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">
                            BIO
                          </div>
                        )}
                        {qte > 0 && (
                          <div className="absolute top-1.5 right-1.5 bg-green-700 text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center shadow-lg">
                            {qte}
                          </div>
                        )}
                        {epuise && (
                      ...
                      <div className="p-3">
                        ...
                        <div className="flex items-center justify-between mb-2">
                          {p.prix_ht > 0 ? (
                            <span className="text-xs text-green-700 font-semibold">
                              {p.prix_ht.toFixed(2)} €/{p.unite}
                            </span>
                          ) : <span />}
                          {p.quantite_dispo != null && p.quantite_dispo > 0 && p.quantite_dispo <= 5 && (
                            <span className="text-[10px] font-bold text-amber-600">
                              ⚡{p.quantite_dispo} restant{p.quantite_dispo > 1 ? 's' : ''}
                            </span>
                          )}
                        </div>
                        ...
                        {/* Bouton / stepper */}
                        {epuise ? (
                          <div className="w-full py-2.5 rounded-xl bg-gray-100 text-gray-400 text-xs font-semibold text-center">
                            Indisponible
                          </div>
                        ) : qte === 0 ? (
                          <button onClick={() => setQte(p.id, 1)}
                            className="w-full py-2.5 rounded-xl bg-green-700 text-white text-sm font-bold active:bg-green-800 transition-colors">
                            + Ajouter
                          </button>
                        ) : (
                          <div className="flex items-center justify-between bg-green-50 rounded-xl p-1 gap-1">
                            <button onClick={() => setQte(p.id, -1)}
                              className="w-10 h-10 flex items-center justify-center bg-white rounded-lg text-green-800 font-bold text-xl shadow-sm active:scale-95 transition-transform">
                              −
                            </button>
                            <span className="font-bold text-green-900 text-lg min-w-[1.5rem] text-center">{qte}</span>
                            <button onClick={() => setQte(p.id, 1)}
                              className="w-10 h-10 flex items-center justify-center bg-green-700 rounded-lg text-white font-bold text-xl shadow-sm active:bg-green-800 active:scale-95 transition-all">
                              +
                            </button>
                          </div>
                        )}
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Instructions particulières, heure de livraison préférée..."
            rows={3}
      ...
      {nbArticles > 0 && (
        <div className="fixed bottom-0 left-0 right-0 px-4 pb-safe-or-4 pt-2 bg-gradient-to-t from-[#F7F5F0] via-[#F7F5F0]/90 to-transparent">
          <div className="max-w-lg mx-auto">
            <button
              onClick={commander}
              disabled={sending}
              className="w-full bg-green-800 text-white py-4 rounded-2xl font-bold text-base shadow-2xl disabled:opacity-60 flex items-center justify-between px-5 active:bg-green-900 transition-colors">
              <span className="bg-green-600 text-white text-sm font-bold w-8 h-8 rounded-full flex items-center justify-center shrink-0">
                {nbArticles}
              </span>
